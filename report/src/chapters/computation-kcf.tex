...
\begin{algorithm}
    \caption{Procedure to compute KCF of an arbitrary pencil.}\label{alg:kcf}
    \KwData{$\Gamma(\lambda) = A + \lambda B$: pencil of \(m \times n\) matrices}
    \KwResult{$K$: Kronecker's canonical form of $\Gamma(\lambda)$}
    \eIf{$A$ is zero and $B$ is zero}{
        \Return{$\Gamma(\lambda)$}\;
    }
    {
        $\tilde{\Gamma}(\lambda) \gets A + \lambda B$\;
        $L, R, transpose \gets I^{(m)}, I^{(n)}, False$\;
        \While{True}{
            \eIf{dimensions($\tilde{A}$) = dimensions($\tilde{B}$) and
